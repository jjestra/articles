# -*- coding: utf-8 -*-
"""
G3SLS Estimation of Peer Effects in Endogenous Social Networks
"""
...
class Netivreg():
    ...
    ----------
    Pi           : array
                   (k+1)x(k+1) array of projection coefficients
    VPi          : array
                   Variance covariance matrix of Pi
    psi_g2sls    : array
                   (k+1)x+1 array of G3SLS coefficients
    Vpsi         : array
                   Variance covariance matrix of psi_g2sls
    N            : integer
                   Number of units in the sample
    K            : integer
                   Number of variables for which psi_g2sls is estimated
                   (including the constant)
    wald         : float
                   Wald Statistic to test H0 that psi_g2sls = 0
    """
...
def sinv(A):
    ...
    if sp.issparse(A):
        n = A.get_shape()[0]
        Ai = sp.linalg.spsolve(A.tocsc(), sp.identity(n, format="csc"))
    else:
        n = A.shape[0]
