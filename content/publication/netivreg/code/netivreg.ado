...
program define netivreg, eclass
    ...
    local depvar `lhs'
    local exovar `exog'
    syntax [anything(name=0)] [if] [in], [WX(string) ID(string) TRansformed CLuster(string) FIRST SECOND]
    tempvar touse
    ...
    python: calc_netivreg("`depvar'", "`exovar'", "`adjmat'", "`adjmat0'", "`wx'", "`id'", "`transformed'", "`cluster'")
...
from function import Netivreg


def calc_netivreg(depvar, exovar, adjmat, adjmat0,
                  wx, id, transformed, cluster):

    id = "id" if id == "" else id

    # Use the sfi Frame class to access data
    if cluster != "":
        list_cols = [id, cluster, depvar] + exovar.split(" ")
    else:
        list_cols = [id, depvar] + exovar.split(" ")
    ...
    if len(set(G.nodes()).difference(df_nodes[id])) > 0:
        print("Warning:", len(set(G.nodes()).difference(df_nodes[id])), "nodes from W matrix not in S matrix.")
    if len(set(G0.nodes()).difference(df_nodes[id])) > 0:
        print("Warning:", len(set(G0.nodes()).difference(df_nodes[id])), "nodes from W0 matrix not in S matrix.")

    cluster_val = None if cluster == "" else cluster
    tr_val = False if transformed == "" else True
    wexovar = [] if wx == "" else wx.split(" ")

    results = Netivreg(name_y=depvar,
                       name_x=exovar.split(" "),
                       name_xs=wexovar,
                       W=W,
                       W0=W0,
...
di
* Line 1
di "Network IV Regression" _continue
